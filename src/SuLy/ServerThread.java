        ...
        try {
            ...
            while (!isClosed) {
                ...
                if(messageSplit[0].equals("win")){
                    userDAO.addWinGame(this.user.getID());// add vào database
                    room.increaseNumberOfGame();// xet game moi
                    room.getCompetitor(clientNumber).write("caro,"+messageSplit[1]+","+messageSplit[2]);
                    room.boardCast("new-game,");//tao game moi
                }
                if(messageSplit[0].equals("lose")){
                    userDAO.addWinGame(room.getCompetitor(clientNumber).user.getID());//them vao database người thua
                    room.increaseNumberOfGame();// xet game moi
                ...
                if(messageSplit[0].equals("draw-confirm")){
                    room.increaseNumberOfDraw();// xet game hoa
                    room.increaseNumberOfGame();// xet game moi
